...
namespace btr.distrib.PurchaseContext.InvoiceInfo
{
    public partial class InvoiceInfoForm : Form
    {
        ...
        private void ExcelButton_Click(object sender, EventArgs e)
        {
            ...
            var listToExcel = new List<InvoiceView>();
            foreach (var item in filtered)
            {
                listToExcel.Add(item.GetData() as InvoiceView);
            }
            ...
            using (IXLWorkbook wb = new XLWorkbook())
            {
                ...
                //  set border and font
                ws.Range(ws.Cell($"A{1}"), ws.Cell($"J{listToExcel.Count + 1}")).Style
                ...
                ws.Range(ws.Cell($"G{2}"), ws.Cell($"J{listToExcel.Count + 1}"))
