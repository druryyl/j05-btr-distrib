        ...
        private void GenStok(OpnameModel opname)
        {
            ...
            if (qtyAdjust > 0)
            {
                var cmd = new AddStokRequest(opname.BrgId, opname.WarehouseId,
                    qtyAdjust, opname.Satuan1, opname.Nilai, opname.OpnameId, "OPNAME", "Stok Adjustment", opname.OpnameDate);
                _addStokWorker.Execute(cmd);
            }
            else
            {
                var cmd = new RemoveFifoStokRequest(opname.BrgId, opname.WarehouseId,
                    -qtyAdjust, opname.Satuan1, 0, opname.OpnameId, "OPNAME", "Stok Adjustment", opname.OpnameDate);
