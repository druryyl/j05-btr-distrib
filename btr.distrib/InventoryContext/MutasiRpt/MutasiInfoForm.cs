...
{
    public partial class MutasiInfoForm : Form
    {
        private readonly IMutasiBrgViewDal _mutasiBrgViewDal;
        private readonly IBrgSatuanDal _brgSatuanDal;
        private List<MutasiBrgView> _dataSource;

        public MutasiInfoForm(IMutasiBrgViewDal returBrgViewDal,
            IBrgSatuanDal brgSatuanDal)
        {
            InitializeComponent();
            _mutasiBrgViewDal = returBrgViewDal;
            _brgSatuanDal = brgSatuanDal;
            InfoGrid.QueryCellStyleInfo += InfoGrid_QueryCellStyleInfo;
            ProsesButton.Click += ProsesButton_Click;
            ExcelButton.Click += ExcelButton_Click;
            _dataSource = new List<MutasiBrgView>();
        ...
        {
            InfoGrid.DataSource = new List<MutasiBrgView>();
            ...
            InfoGrid.TableDescriptor.Columns["MutasiDate"].Appearance.AnyRecordFieldCell.Format = "dd-MMM-yyyy";
        ...
        private void Proses()
        {
            ...
            var listFaktur = _mutasiBrgViewDal.ListData(periode)?.ToList() ?? new List<MutasiBrgView>();
            listFaktur = listFaktur
                .OrderBy(x => x.MutasiDate.Date)
                .ToList();
            ...
            _dataSource = Filter(listFaktur, BrgText.Text);
            _dataSource.ForEach(x => x.MutasiDate = x.MutasiDate.Date);
            InfoGrid.DataSource = _dataSource;
        }

        private static List<MutasiBrgView> Filter(List<MutasiBrgView> source, string keyword)
        {
            if (keyword.Trim().Length == 0)
                return source;
            var result = source.Where(x => x.BrgName.ToLower().ContainMultiWord(keyword)).ToList();
