...
namespace btr.distrib.PrintDocs
{
    ...
    public class FakturPrintDoc : IFakturPrintDoc
    {
        ...
        public async void CreateDoc2(FakturModel model)
        {
        ...
        private List<FakturItemModel> PindahBonusNewBaris(List<FakturItemModel> listItem)
        {
            ...
            {
                result.Add(item);
                if (item.ListQtyHarga.FirstOrDefault(x => x.NoUrut == 3) == null)
                    continue;
                var itemBonus = item.ListQtyHarga.FirstOrDefault(x => x.NoUrut == 3) ?? new FakturQtyHargaModel();
                item.ListQtyHarga.RemoveAll(x => x.NoUrut == 3);
