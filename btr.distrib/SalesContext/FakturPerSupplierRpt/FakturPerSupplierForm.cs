...
namespace btr.distrib.SalesContext.FakturPerSupplierRpt
{
    public partial class FakturPerSupplierForm : Form
    {
        private readonly IFakturPerSupplierDal _fakturPerSupplierDal;
        private List<FakturPerSupplierView> _dataSource;

        public FakturPerSupplierForm(IFakturPerSupplierDal fakturPerSupplierDal)
        {
            InitializeComponent();
            _fakturPerSupplierDal = fakturPerSupplierDal;
        ...
        private void InitGrid()
        {
            ...
            InfoGrid.TableDescriptor.Columns["QtyJual"].Appearance.AnyRecordFieldCell.BackColor = Color.LightGreen;
        ...
        private void ExcelButton_Click(object sender, EventArgs e)
        {
            ...
            using (var saveFileDialog = new SaveFileDialog())
            {
                ...
                saveFileDialog.FileName = $"faktur-per-supplier-info-{DateTime.Now:yyyy-MM-dd-HHmm}";
            ...
            var listToExcel = new List<FakturPerSupplierView>();
            foreach(var item in filtered)
            {
                listToExcel.Add(item.GetData() as FakturPerSupplierView);
            }

            using (IXLWorkbook wb = new XLWorkbook())
            {
                wb.AddWorksheet("faktur-per-supplier-info")
                .Cell($"B1")
                    .InsertTable(listToExcel, false);
                var ws = wb.Worksheets.First();
                
                //  set format row header: font bold, background lightblue, border medium
                ws.Range(ws.Cell("A1"), ws.Cell($"AB1")).Style
                ...
                ws.Range(ws.Cell("A2"), ws.Cell($"AB{listToExcel.Count + 1}")).Style
                    .Font.SetFontName("Lucida Console")
                    .Font.SetFontSize(9)
                    .Border.SetOutsideBorder(XLBorderStyleValues.Medium)
                    .Border.SetInsideBorder(XLBorderStyleValues.Hair);
                ...
                ws.Range(ws.Cell("A2"), ws.Cell($"AB{listToExcel.Count + 1}"))
                    .Style.NumberFormat.Format = "#,##";

                //  format numeric column  
                ws.Range(ws.Cell("G2"), ws.Cell($"AB{listToExcel.Count + 1}"))
                    .Style.NumberFormat.Format = "#,##";
                ...
                ws.Range(ws.Cell("U2"), ws.Cell($"X{listToExcel.Count + 1}"))
                    .Style.NumberFormat.Format = "#,##0.00_);(#,##0.00);-";

                //  set backcolor numeric column
                ws.Range(ws.Cell("O2"), ws.Cell($"T{listToExcel.Count + 1}"))
                    .Style.Fill.SetBackgroundColor(XLColor.LightGreen);
                ws.Range(ws.Cell("U2"), ws.Cell($"Y{listToExcel.Count + 1}"))
                    .Style.Fill.SetBackgroundColor(XLColor.LightYellow);
                ws.Range(ws.Cell("Z2"), ws.Cell($"AB{listToExcel.Count + 1}"))
                ...
                ws.Cell($"Z{listToExcel.Count + 2}").FormulaA1 = $"=SUM(Z2:Z{listToExcel.Count + 1})";
                ws.Cell($"AA{listToExcel.Count + 2}").FormulaA1 = $"=SUM(AA2:AA{listToExcel.Count + 1})";
                ws.Cell($"AB{listToExcel.Count + 2}").FormulaA1 = $"=SUM(AB2:AB{listToExcel.Count + 1})";

                //  format row footer font bold, background yellow, border medium
                ws.Range(ws.Cell($"T{listToExcel.Count + 2}"), ws.Cell($"AB{listToExcel.Count + 2}")).Style
        ...
        private static IEnumerable<FakturPerSupplierView> Filter(IReadOnlyCollection<FakturPerSupplierView> source, string keyword)
        {
            ...
            var listilterFakturCode = source.Where(x => x.FakturCode.ToLower().StartsWith(keyword.ToLower())).ToList();

            var result = listFilteredBrgName
                .Union(listFilteredBrgCode)
                .Union(listFilteredSupplier)
                .Union(listFilteredCustomer)
                .Union(listilterFakturCode);
