...
namespace btr.infrastructure.SalesContext.FakturCashRpt
{
    internal class FakturCashViewDal : IFakturCashViewDal
    {
        private readonly DatabaseOptions _opt;

        public FakturCashViewDal(IOptions<DatabaseOptions> opt)
        {
            _opt = opt.Value;
        }

        public IEnumerable<FakturCashView> ListData(Periode filter)
        {
            const string sql = @"
                SELECT 
                    aa.FakturId, aa.FakturCode, aa.FakturDate as Tgl, 
                    aa.Total, aa.Discount, aa.Tax, aa.GrandTotal, aa.UangMuka as Cash, aa.KurangBayar,
            ...
            {
                return conn.Read<FakturCashView>(sql, dp);
            }
        }
    }
}
